...
//-----------------------------------------------------------------------------
// Any executable that wants to use con_vars need to implement one of
...
class ConCommandBase
{
    ...
    friend void con_var_Register(int nCVarFlag, IConCommandBaseAccessor* pAccessor);
...
public:
    ...
    // Prior to register, it points to the next con_var in the DLL.
    // Once registered, though, m_pNext is reset to point to the next
    // con_var in the global list
...
protected:
    // con_vars add themselves to this list for the executable. 
    // Then con_var_Register runs through  all the console variables 
    // and registers them into a global list stored in vstdlib.dll
    static ConCommandBase* s_pConCommandBases;

    // con_vars in this executable use this 'global' to access values.
    static IConCommandBaseAccessor* s_pAccessor;

public:
    // This list will hold all the registered commands.
    // It is not from the official SDK. I've added this so that
    // we can parse all con_vars we have created if we want to
...
//-----------------------------------------------------------------------------
class con_var : public ConCommandBase, public i_convar
{
    friend class CCvar;
    friend class con_varRef;
    friend class SplitScreencon_varRef;
...
public:
    ...
    con_var(const char* pName, const char* pDefaultValue, int flags, const char* pHelpString, bool bMin, float fMin, bool bMax, float fMax, FnChangeCallback_t callback);
...
// Output : int
//-----------------------------------------------------------------------------
FORCEINLINE_CVAR int con_var::GetInt(void) const
{
...
// Output : Color
//-----------------------------------------------------------------------------
FORCEINLINE_CVAR color con_var::GetColor(void) const
{
    int value = GetInt();
...
template <> FORCEINLINE_CVAR float          con_var::Get<float>(void) const { return get_float(); }
template <> FORCEINLINE_CVAR int            con_var::Get<int>(void) const { return GetInt(); }
template <> FORCEINLINE_CVAR bool           con_var::Get<bool>(void) const { return GetBool(); }
template <> FORCEINLINE_CVAR const char* con_var::Get<const char*>(void) const { return GetString(); }
template <> FORCEINLINE_CVAR float          con_var::Get<float>(float* p) const { return (*p = get_float()); }
template <> FORCEINLINE_CVAR int            con_var::Get<int>(int* p) const { return (*p = GetInt()); }
template <> FORCEINLINE_CVAR bool           con_var::Get<bool>(bool* p) const { return (*p = GetBool()); }
template <> FORCEINLINE_CVAR const char* con_var::Get<const char*>(char const** p) const { return (*p = GetString()); }

//-----------------------------------------------------------------------------
// Purpose: Return con_var value as a string, return "" for bogus string pointer, etc.
// Output : const char *
//-----------------------------------------------------------------------------
FORCEINLINE_CVAR const char* con_var::GetString(void) const
{
...
//-----------------------------------------------------------------------------
void con_var_Register(int nCVarFlag = 0, IConCommandBaseAccessor* pAccessor = NULL);
void con_var_Unregister();